...
WeightT FindShortestPathWithSaving(IGraph& graph, std::vector<VertexT>& path, const VertexT& start,
                                   const VertexT& end) {
    ...
    while (!temp_queue.empty()) {
        ...
        for (auto edge : graph.GetEdges(current_vertex)) {
            ...
            if (dist[current_neighbour] == IGraph::kUNDEFINED && current_max_flow - current_flow > 0) {
                dist[current_neighbour] = std::min(dist[current_vertex], current_max_flow - current_flow);
    ...
    answer = dist[end];
    if (answer == IGraph::kUNDEFINED) {
        return {};
    }

    path.push_back(end);
    VertexT current = prev[end];
    while (current != start) {
...
WeightT FindMaxFlowEdmondsKarp(const GraphList& graph, const VertexT source, const VertexT destination) {
    WeightT result = 0;
...
int main() {
    ...
    std::cout << FindMaxFlowEdmondsKarp(graph, 1, vertexes_num) << std::endl;
